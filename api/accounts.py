...
async def get_accounts(
    ...
    logger.info(f"Запрос счетов для client_id={target_client_id}, найдено счетов: {len(accounts)}")
    for acc in accounts:
        logger.info(f"  - Счет ID={acc.id}, номер={acc.account_number}, баланс={acc.balance}")
...
async def get_balances(
    ...
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"Запрос баланса для account_id={account_id} (ID={acc_id}), номер={account.account_number}, баланс={account.balance}")
...
class CreateAccountSimpleRequest(BaseModel):
    """Упрощенный запрос на создание счета для фронтенда"""
    account_name: str
    currency: str = "RUB"
    balance: float = 0
...
@router.post("/create", summary="Создать счет (упрощенный)", tags=["2 Счета и балансы"])
async def create_account_simple(
    request: CreateAccountSimpleRequest,
    current_client: dict = Depends(get_current_client),
    db: AsyncSession = Depends(get_db)
):
    """
    Упрощенное создание нового счета для фронтенда
    ...
    # Генерация реалистичного номера карты российского банка
    # BIN коды российских банков:
    # 220х хх - Мир (Сбербанк, ВТБ и др.)
    # 427х хх - Visa (ВТБ, Альфа и др.) 
    # 546х хх - Mastercard (российские банки)
    
    import random
    
    # Выбираем случайный BIN
    bin_options = [
        "2200",  # Мир Сбербанк
        "2201",  # Мир ВТБ
        "2202",  # Мир Альфа
        ...
        "4272",  # Visa Сбербанк
        "5469",  # Mastercard ВТБ
        "5536",  # Mastercard Сбербанк
    ]
    
    chosen_bin = random.choice(bin_options)
    
    # Генерируем остальные 12 цифр
    card_digits = chosen_bin + ''.join([str(random.randint(0, 9)) for _ in range(12)])
    
    # Используем как номер счета
