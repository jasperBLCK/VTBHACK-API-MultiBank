...
export default function Header({ notifications = [], onMarkAsRead = () => {}, onRemoveNotification = () => {}, onMenuClick, onCreateAccount }: HeaderProps) {
    ...
    <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 sticky top-0 z-30 w-full">
      <div className="w-full px-3 sm:px-4 lg:px-6 xl:px-8">
        <div className="flex justify-between items-center h-14 sm:h-16 w-full">
          ...
          {/* Правая часть: Создать счет + Уведомления + Настройки + Аватар */}
          <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
            {onCreateAccount && (
              <button
                onClick={onCreateAccount}
                className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 transition-all font-medium text-sm shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 hover:scale-[1.02] active:scale-[0.98]"
              >
            ...
            {onCreateAccount && (
              <button
                onClick={onCreateAccount}
                className="sm:hidden p-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 transition-all shadow-lg shadow-purple-500/30"
              >
