...
export async function POST(req: NextRequest) {
  try {
    ...
    const { account_id, bank_url, bank_token, consent_id } = body
    ...
    const response = await fetch(
      `${API_BASE_URL}/multibank/transactions-with-consent`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ account_id, bank_url, bank_token, consent_id }),
      }
    )
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.detail || errorData.message || 'Ошибка получения транзакций' },
